...
export default function CheckoutClient() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      router.push('/auth/login?redirect=/checkout?event=' + eventId);
      return;
    }
  ...
  const loadEventAndTickets = async () => {
    try {
      ...
      // Load wallet balance from user document
      if (user) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (selectedPayment === 'pagali') {
        ...
        // Construct webhook URL - use Cloud Function directly
        ...
        await payWithWalletFn({
          orderId,
          useBonusBalance: true, // Use bonus balance if available
        });
